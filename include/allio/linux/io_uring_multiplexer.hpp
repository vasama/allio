...
private:
	...
	using unique_sqe_index = detail::unique_resource<uint32_t, sqe_index_deleter>;
...
public:
	...
	public:
		result<void> push(auto&& initialize)
		{
			allio_TRY(sqe, acquire_sqe());
			...
			allio_ASSERT(sqe_view->user_data == 0);
			...
			allio_ASSERT((sqe_view->flags & ~user_flags) != 0);

			return {};
		}

		result<void> push_linked_timeout(deadline const deadline);
	...
	private:
		...
		{
			allio_ASSERT((std::exchange(m_multiplexer->m_flags, m_multiplexer->m_flags | submit_lock) & submit_lock) == 0);
		}
		...
		result<io_uring_sqe*> acquire_sqe();

		friend class io_uring_multiplexer;
	};

	result<void> submit(async_operation_storage& operation, auto&& submit)
	{
		submit_context context(*this, operation);

		result<void> r = static_cast<decltype(submit)&&>(submit)(context);
	...
	result<void> push_linked_timeout();


private:
	io_uring_sqe* get_sqe(uint32_t const index)
	{
		allio_ASSERT(index < m_sq_size); // Mask first.
	...
	result<void> enter(pump_mode mode, deadline deadline);
};
