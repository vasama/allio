...
public:
	...
	result<void> start(async_operation_storage& operation, Callable&& callable)
	{
...
private:
	result<void> post_result(async_operation_storage& operation, result<void> const result)
	{
		set_result(operation, as_error_code(result));
		post(operation, async_operation_status::concluded);
		return {};
	}

	result<void> post_result(async_operation_storage& operation, async_result<void> const result)
	{
		if (!result)
		{
			...
			{
				return allio_ERROR(error.error_code());
			}
