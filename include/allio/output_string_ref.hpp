...
class output_string_ref
{
	...
	typedef result<buffer> resize_callback(output_string_ref const& self, size_t size, bool oversize);
...
public:
	...
	result<std::span<char>> resize_utf8(size_t const size, bool const oversize = false) const
	{
	...
	explicit output_string_ref(Container& container, bool const c_string)
		...
		, m_resize(container_resize<Container>)
	{
	}
	...
	result<std::span<wchar_t>> resize_wide(size_t const size, bool const oversize = false) const
	{
...
private:
	...
	result<std::span<Char>> resize_internal(size_t const size, bool const oversize) const
	{
		allio_TRY(buffer, m_resize(*this, size, oversize));
		return std::span(reinterpret_cast<Char*>(buffer.data), buffer.size);
	}

	template<typename Char>
	static result<buffer> resize_buffer(output_string_ref const& self, size_t const size, bool const oversize)
	{
	...
	static result<buffer> resize_container(output_string_ref const& self, size_t const size, bool const oversize)
	{
