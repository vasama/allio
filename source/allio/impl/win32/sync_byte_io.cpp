...
static result<size_t> sync_scatter_gather_io(platform_handle const& h, io::scatter_gather_parameters const& args, decltype(win32::NtReadFile) const syscall)
{
	if (!h)
	{
	...
	file_offset offset = args.offset;
...
result<size_t> allio::sync_scatter_read(platform_handle const& h, io::scatter_gather_parameters const& args)
{
	return sync_scatter_gather_io(h, args, win32::NtReadFile);
}

result<size_t> allio::sync_gather_write(platform_handle const& h, io::scatter_gather_parameters const& args)
{
