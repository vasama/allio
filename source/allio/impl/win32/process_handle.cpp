...
public:
	static result<wchar_t*> make_command_line(command_line_storage& buffer,
...
private:
	...
	result<wchar_t*> resize_buffer(size_t const new_data_size)
	{
		...
		{
			return allio_ERROR(error::process_arguments_too_long);
		}
	...
	result<wchar_t*> reserve(size_t const size)
	{
	...
	result<void> push_argument(std::wstring_view const argument)
	{
		...
		allio_TRY(buffer_beg, reserve(required_size));
		...
		*buffer_beg++ = L' ';
		...
		allio_ASSERT(buffer_beg == buffer_end);

		return {};
	}

	result<void> push_argument(std::string_view const argument)
	{
		wchar_t* wide_beg = m_buffer_pos;
		allio_TRY(wide_size, [&]() -> result<size_t>
		{
			while (true)
			{
				...
				allio_TRYA(wide_beg, resize_buffer(0));
			}
		}());
		...
		if (info.extra_size != 0)
		{
			...
			{
				allio_ASSERT(info.extra_size == 2 && info.requires_quotes);
			...
			allio_ASSERT(shift_end == m_buffer_pos);
	...
	result<void> push_argument(input_string_view const argument)
	{
		allio_ASSERT(!argument.is_too_long());
		return argument.is_wide()
			? push_argument(argument.wide_view())
			: push_argument(argument.utf8_view());
	}

	result<wchar_t*> make_command_line(input_string_view const program, std::span<input_string_view const> const arguments)
	{
		allio_TRYV(push_argument(program));
		for (input_string_view const argument : arguments)
		{
			...
			allio_TRYV(push_argument(argument));
		}
		...
		allio_ASSERT(m_buffer_pos != buffer_beg);
		m_buffer_pos[-1] = L'\0';
...
static size_t escape_in_place(std::span<wchar_t> const string, size_t const escape_count)
{
	...
	allio_ASSERT(out == beg);

	return string.size() + escape_count;
}

static result<wchar_t*> make_command_line(command_line_storage& buffer, std::span<input_string_view const> const arguments)
{
	...
	for (size_t utf8_index = 0; input_string_view const argument : arguments)
	{
		...
		else
		{
			...
			allio_TRY(wide_size, utf8_to_wide(utf8_argument));
	...
	allio_TRY(buffer_beg, buffer.reserve(total_size));
	...
	*buffer_pos++ = L' ';
	...
	allio_ASSERT(static_cast<size_t>(buffer_pos - buffer_beg) == total_size);

	return buffer_beg;
}
#endif

static result<process_exit_code> get_process_exit_code(HANDLE const handle)
{
	DWORD exit_code;
...
result<unique_handle> win32::open_process(process_id const pid, process_handle::open_parameters const& args)
{
	...
	return result<unique_handle>(result_value, handle);
}

result<process_info> win32::launch_process(input_path_view const path, process_handle::launch_parameters const& args)
{
	...
	allio_TRY(command_line, make_command_line(command_line_storage, path, args.arguments));
	...
	return result<win32::process_info>(result_value, process_info.hProcess, static_cast<process_id>(process_info.dwProcessId));
}

#if 0
static result<unique_handle> duplicate_handle(HANDLE const source_process, HANDLE const source_handle)
{
	...
	return result<unique_handle>(result_value, target_handle);
}
#endif
...
process_handle detail::process_handle_base::current()
{
	...
	allio_VERIFY(process.set_native_handle(native));
	return process;
}

result<void> detail::process_handle_base::close_sync(basic_parameters const& args)
{
	if (get_flags()[implementation::flags::pseudo_handle])
	{
		allio_TRYD(platform_handle::release_native_handle());
	}
	else
	{
		allio_TRYV(platform_handle::close_sync(args));
	}
...
result<void> detail::process_handle_base::sync_impl(io::parameters_with_result<io::process_open> const& args)
{
	...
	allio_TRY(process, win32::open_process(args.pid, args));
	return initialize_platform_handle(h, std::move(process),
...
result<void> detail::process_handle_base::sync_impl(io::parameters_with_result<io::process_launch> const& args)
{
	process_handle& h = *args.handle;
	...
	allio_TRY(process, win32::launch_process(args.path, args));
	return initialize_platform_handle(h, std::move(process.handle),
		[&](native_platform_handle const handle)
		{
			return process_handle::native_handle_type
			{
...
result<void> detail::process_handle_base::sync_impl(io::parameters_with_result<io::process_wait> const& args)
{
	process_handle& h = *args.handle;
	...
	if (!h.get_flags()[flags::exited])
	{
		...
		allio_TRYA(h.m_exit_code.value, get_process_exit_code(handle));
