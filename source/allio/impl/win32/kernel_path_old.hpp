...
public:
	...
	static result<basic_kernel_path<Context>> make_path(
		...
		basic_kernel_path_parameters<Context> const& args, Context& context)
	{
		...
		allio_ASSERT(!(args.win32_api_form && args.handle));

		if (args.path.is_too_long())
		{
			return allio_ERROR(error::filename_too_long);
		}

		if (args.path.empty())
		{
			return allio_ERROR(error::invalid_path);
		}

		basic_kernel_path_converter c(storage, args, context);
		allio_TRYV(c.make_path(args.path));
...
private:
	...
	result<wchar_t*> resize_buffer(size_t const new_data_size)
	{
	...
	result<void> push_literal(char const* const beg, char const* const end)
	{
		if (size_t const utf8_size = end - beg)
		{
			std::string_view const utf8_string = std::string_view(beg, utf8_size);

			allio_TRY(wide_size, utf8_to_wide(utf8_string));
			...
			{
				allio_TRYA(new_buffer_end, resize_buffer(wide_size));
			}

			allio_VERIFY(utf8_to_wide(utf8_string, std::span(new_buffer_end, wide_size)));
		}
		return {};
	}

	result<void> push_literal(wchar_t const* const beg, wchar_t const* const end)
	{
		if (size_t const size = end - beg)
		{
			...
			else
			{
				...
				{
					allio_TRY(new_buffer_end, resize_buffer(size));
					memcpy(new_buffer_end, beg, size * sizeof(wchar_t));
				}
			}
		}
	...
	template<typename Char>
	result<void> push_literal(std::basic_string_view<Char> const string)
	{
	...
	template<typename Char>
	result<size_t> push_canonical_impl(Char const* const beg, Char const* end, size_t backtrack, auto const push_literal)
	{
		bool need_separator = m_buffer_pos != m_buffer_end && *m_buffer_pos != Char('\\');

		Char const* flush_end = end;
		auto const flush = [&](Char const* const flush_beg, Char const* const new_flush_end) -> result<void>
		{
			if (need_separator && flush_beg != flush_end && flush_end[-1] != Char('\\'))
			{
				wchar_t const* const separator = L"\\";
				allio_TRYV(push_literal(separator, separator + 1));
		...
		while (beg != end)
		{
			...
			if (segment_beg[0] == '.')
			{
				...
				if (segment_size == 1)
				{
					allio_TRYV(flush(separator_end, segment_beg));
					continue;
				}
			...
			if (size_t const separator_size = separator_end - separator_beg)
			{
				...
				{
					allio_TRYV(flush(separator_end, segment_end));
		...
		allio_TRYV(flush(beg, beg));

		return backtrack;
	}

	template<typename Char>
	result<size_t> push_canonical(Char const* const beg, Char const* const end, size_t backtrack = 0)
	{
		return push_canonical_impl(beg, end, backtrack,
			[this]<typename C>(C const* const beg, C const* const end) -> result<void>
			{
	...
	template<typename Char>
	result<void> push_canonical_literal(Char const* const beg, Char const* const end)
	{
		bool first_segment = true;
		allio_TRYD(push_canonical_impl(beg, end, 0,
			[this, &first_segment]<typename C>(C const* const beg, C const* const end) -> result<void>
			{
				if (first_segment)
				{
					first_segment = false;
	...
	result<void> push_drive_root(wchar_t const drive)
	{
	...
	result<current_path> classify_current_path(path_section<wchar_t> const path)
	{
		...
		if (beg == end)
		{
			return allio_ERROR(error::invalid_current_directory);
		}
	...
	static result<wchar_t> detect_current_drive(path_section<wchar_t> const path)
	{
		wchar_t const* beg = path.beg;
		wchar_t const* const end = path.end;
	...
	//    ^
	template<typename Char>
	result<void> set_drive_absolute_path(Char const* beg, Char const* const end, wchar_t const drive)
	{
		allio_TRYD(push_canonical(beg, end));
		allio_TRYV(push_drive_root(drive));
	...
	//     ^        ^
	template<typename Char>
	result<void> set_local_device_path(Char const* const beg, Char const* const end, bool const canonicalize)
	{
		if (canonicalize)
		{
			allio_TRYD(push_canonical(beg, end));
		...
		{
			allio_TRYV(push_canonical_literal(beg, end));
		}

		allio_TRYV(push_literal(std::wstring_view(L"\\??\\")));
	...
	//   ^
	template<typename Char>
	result<void> set_unc_path(Char const* beg, Char const* const end)
	{
		...
		allio_TRYD(push_canonical(skip_separators(beg, end), end));
		allio_TRYD(push_canonical(server_beg, share_end));
		allio_TRYV(push_literal(std::wstring_view(L"\\??\\UNC\\")));
	...
	//  ^
	template<typename Char>
	result<void> set_absolute_path(Char const* const beg, Char const* const end)
	{
		if (m_handle)
		{
		...
		allio_TRY(current_drive, detect_current_drive(current.path));

		allio_TRYD(push_canonical(beg, end));
		allio_TRYV(push_drive_root(current_drive));
	...
	//   ^
	template<typename Char>
	result<void> set_drive_relative_path(Char const* const beg, Char const* const end, wchar_t const drive)
	{
		if (m_handle)
		{
		...
		m_lock = m_context.lock();
		auto const current = m_context.get_current_directory(m_lock);
		allio_TRY(classification, classify_current_path(current.path));
		...
		return set_relative_from_current(beg, end, current_handle, [&]() -> result<current_path>
		{
			if (is_current_drive)
			{
			...
			allio_TRY(classification_on_drive, classify_current_path(*current_on_drive));
	...
	// ^
	template<typename Char>
	result<void> set_relative_path(Char const* const beg, Char const* const end)
	{
		return m_handle
	...
	template<typename Char>
	result<void> set_relative_from_handle(Char const* const beg, Char const* const end)
	{
		allio_TRY(backtrack, push_canonical(beg, end));
	...
	template<typename Char>
	result<void> set_relative_from_current(Char const* const beg, Char const* const end)
	{
		m_lock = m_context.lock();
		auto const current = m_context.get_current_directory(m_lock);
		return set_relative_from_current(beg, end, current.handle, [&]() -> result<current_path>
		{
		...
		handle_type const current_handle, auto const get_current_path)
	{
		...
		else
		{
			...
			allio_TRY(current, get_current_path());
			allio_TRYA(backtrack, push_canonical(current.relative.beg, current.relative.end, backtrack));
			...
			allio_TRYD(push_canonical(current.dyn_root.beg, current.dyn_root.end));
			allio_TRYV(push_literal(current.root.beg, current.root.end));
		}
	...
	template<typename Char>
	result<void> make_path(std::basic_string_view<Char> const string)
	{
		Char const* const beg = string.data();
	...
	result<void> make_path(input_path_view const path)
	{
		if (m_win32_api_form && !path.is_c_string())
		{
			allio_TRYV(push_literal(std::wstring_view(L"\0")));
		}
...
template<typename Context>
result<basic_kernel_path<Context>> make_kernel_path(
	basic_kernel_path_storage<Context>& storage,
...
struct peb_context
{
	...
	public:
		...
		void lock()
		{
			allio_ASSERT(!m_owns_lock.value);
	...
	{
		allio_ASSERT(L'A' <= drive && drive <= L'Z');
