...
static result<kernel_path> rtl_convert_path(rtl_kernel_path_storage& storage, wchar_t const* const win32_path)
{
...
static result<kernel_path> rtl_make_kernel_path(rtl_kernel_path_storage& storage, kernel_path_parameters const& args)
{
	...
	case RtlPathTypeRelative:
		{
			...
			allio_TRYD(rtl_convert_path(storage, string.c_str()));
	...
	return result<kernel_path>(result_value);
}

static result<void> fuzz_main(uint8_t const* data_beg, uint8_t const* data_end)
{
