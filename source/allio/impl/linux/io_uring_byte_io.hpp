...
public:
	...
	result<void> submit(io_uring_multiplexer& m, uint8_t const opcode, bool const supports_cancellation = true)
	{
		return m.submit(*this, [&](io_uring_multiplexer::submit_context& ctx) -> result<void>
		{
			if (supports_cancellation && !m_args.deadline.is_trivial())
			{
				allio_TRYV(ctx.push_linked_timeout(m_args.deadline));
			}
	...
	result<void> cancel(io_uring_multiplexer& m)
	{
