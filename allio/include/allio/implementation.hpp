...
struct multiplexer_handle_translation
{
	...
	static vsm::result<void> register_handle(multiplexer& m, handle const& h)
	{
		return implementation::register_handle(static_cast<M&>(m), static_cast<H const&>(h));
	}

	static vsm::result<void> deregister_handle(multiplexer& m, handle const& h)
	{
...
struct multiplexer_handle_operation_translation<M, H, O, 1>
{
	...
	static vsm::result<async_operation_ptr> construct(multiplexer& m, storage_ptr const storage, async_operation_parameters const& a, async_operation_listener* const listener)
	{
		...
		{
			return vsm::result<async_operation_ptr>(vsm::result_value, new (storage) storage_type(static_cast<parameters_type const&>(a), listener), destroy);
		}
	}

	static vsm::result<void> start(multiplexer& m, async_operation& s)
	{
		return implementation::start(static_cast<M&>(m), static_cast<storage_type&>(s));
	}

	static vsm::result<async_operation_ptr> construct_and_start(multiplexer& m, storage_ptr const storage, async_operation_parameters const& a, async_operation_listener* const listener)
	{
		if constexpr (requires { implementation::construct_and_start; })
		{
			return vsm::result<async_operation_ptr>(vsm::result_value,
				implementation::construct_and_start(static_cast<M&>(m), storage, static_cast<parameters_type const&>(a), listener),
				destroy);
		}
		else
		{
			vsm_try_result(r, construct(m, storage, a, listener));
			vsm_try_void(start(m, **r));
			return r;
		}
	}

	static vsm::result<void> cancel(multiplexer& m, async_operation& s)
	{
	...
	static vsm::result<void> block(multiplexer& m, async_operation_parameters const& a)
	{
		...
		else
		{
			...
			vsm_try(operation, construct_and_start(m, storage_ptr(storage, sizeof(storage)), a, nullptr));
...
vsm::result<multiplexer_handle_relation const*> find_handle_relation(
...
public:
	static vsm::result<multiplexer_handle_relation const*> find_handle_relation(type_id<handle> const handle_type)
	{
