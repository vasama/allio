...
private:
	...
	friend vsm::result<io_result> tag_invoke(submit_io_t, queue_multiplexer& m, H&& h, C&& c, S&& s, R&& r)
	{
		operation_type& my_operation = s;
		typename Multiplexer::operation_type& base_operation = my_operation;

		vsm::result<io_result> r = submit_io(m.m_multiplexer, c, s, r);
	...
	template<typename H, typename C, std::derived_from<operation_type> S, typename R>
	friend vsm::result<io_result> tag_invoke(notify_io_t, queue_multiplexer& m, H&& h, C&& c, S&& s, R&& r, io_status const status)
	{
		operation_type& my_operation = s;
		typename Multiplexer::operation_type& base_operation = my_operation;

		vsm::result<io_result> r = notify_io(m.m_multiplexer, c, s, r, status);
		if (!r && r.error() == std::error_code(error::too_many_concurrent_async_operations))
		{
			m_queue.push_back(&my_operation);
			r = std::nullopt;
