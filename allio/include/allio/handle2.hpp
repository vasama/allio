...
protected:
	...
	handle(unchecked_tag, native_handle_type const& native)
	...
	{
		//template<parameters<close_parameters> P = close_parameters::interface>
		//basic_sender<close_tag> close(P const& args = {});
	};
};
...
class basic_handle final
	: public H
	, public H::sync_interface<basic_handle>
{
	template<typename M>
...
struct basic_multiplexer_context
{
	M multiplexer;
	C context;
};
...
class basic_async_handle final
	: public H
	, public H::sync_interface<basic_async_handle>
	, public H::async_interface<basic_async_handle>
{
	using handle_type = basic_handle<H>;

	using multiplexer_handle_type = typename M::handle_type;

	using multiplexer_context_type = basic_multiplexer_context<
		multiplexer_handle_type,
		async_handle_context<H, M>>;
...
public:
	...
	vsm::result<void> set_native_handle(typename H::native_handle_type const& native, M const& multiplexer) &
	{
		...
		handle_type handle(unchecked_tag(), native);
		...
		if (!r)
		{
			...
			handle.release_native_handle(unchecked_tag());
		}
