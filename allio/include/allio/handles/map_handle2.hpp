...
#include <allio/handles/section_handle2.hpp>

namespace allio {
namespace detail {

class map_handle_impl : public handle
{
protected:
...
protected:
	allio_detail_default_lifetime(map_handle_impl);

	explicit map_handle_impl(native_handle_type const& native)
		: base_type(native)
		, m_section(unchecked_tag(), native.section)
		, m_base(native.base)
		, m_size(native.size)
...
using map_handle = basic_handle<detail::map_handle_impl>;
