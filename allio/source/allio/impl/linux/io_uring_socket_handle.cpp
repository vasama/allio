...
struct allio::multiplexer_handle_operation_implementation<io_uring_multiplexer, Handle, io::socket_create>
{
	...
	static vsm::result<void> start(io_uring_multiplexer& m, async_operation_storage& s)
	{
		...
		vsm_try(socket, create_socket(s.address_kind, s.args.handle_flags | flags::multiplexable));
		vsm_try_void(consume_socket_handle(*s.handle, { s.args.handle_flags }, vsm_move(socket)));
...
struct allio::multiplexer_handle_operation_implementation<io_uring_multiplexer, stream_socket_handle, io::socket_connect>
{
	...
	{
		vsm_assert(*s.handle);
		vsm_try_assign(s.addr, socket_address::make(s.address));
...
struct allio::multiplexer_handle_operation_implementation<io_uring_multiplexer, listen_socket_handle, io::socket_listen>
{
	...
	{
		vsm_assert(*s.handle);
		vsm_try_void(listen_socket(unwrap_socket(s.handle->get_platform_handle()), s.address, s.args));
		m.post_synchronous_completion(s);
		return {};
	}
};
...
struct allio::multiplexer_handle_operation_implementation<io_uring_multiplexer, listen_socket_handle, io::socket_accept>
{
	...
	static vsm::result<void> start(io_uring_multiplexer& m, async_operation_storage& s)
	{
		return m.push(s, +[](async_operation_storage& s, io_uring_sqe& sqe)
		{
			...
			{
				vsm_assert(!s.result->socket);
				vsm_try_void(consume_socket_handle(s.result->socket, { s.create_args.handle_flags }, unique_socket(result)));
