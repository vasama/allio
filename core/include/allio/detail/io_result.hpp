...
namespace io_result_state
{
	...
	inline constexpr uint8_t canceled               = 1 << 2;
}
...
public:
	explicit io_canceled(auto&& error)
		: m_error(vsm_forward(error))
	{
	}
...
public:
	...
	template<any_cvref_of_template<io_canceled> U>
		requires std::is_constructible_v<E, unexpected_value_t<U>>
	explicit(!std::is_convertible_v<unexpected_value_t<U>, E>)
	io_result_base(U&& canceled)
		: m_error(vsm_forward(canceled).error())
		, m_state(io_result_state::canceled)
	{
	}
	...
	template<any_cvref_of_template<io_canceled> U>
		requires std::is_convertible_v<unexpected_value_t<U>, E>
	io_result_base& operator=(U&& canceled) &
	{
		assign_error(vsm_forward(canceled).error());
		m_state = io_result_state::canceled;
		return *this;
	}
	...
	[[nodiscard]] bool is_canceled() const
	{
		return (m_state & io_result_state::canceled) != 0;
	}
