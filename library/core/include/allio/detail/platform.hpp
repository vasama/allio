...
vsm_gnu_diagnostic(ignored "-Wold-style-cast")

inline native_platform_handle wrap_handle(platform_handle_type const handle)
{
	return static_cast<native_platform_handle>(
		(platform_handle_uint_type)(handle) + platform_handle_offset);
}

inline platform_handle_type unwrap_handle(native_platform_handle const handle)
{
	return (platform_handle_type)(
		static_cast<platform_handle_uint_type>(handle) - platform_handle_offset);
}
